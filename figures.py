...
NUM_VARS = 13
VAR_DICT = {0 : "N Demand (N\u2080)",
1 : "Reactor Volume (V)",
2 : "Fixation Rate (r)",
3 : "Bioreactor Harvest Index (TODO)",
4 : "Crop Harvest Index (TODO)",
5 : "Fertilization Efficiency",
6 : "Digester (AE) Efficiency (TODO)",
7 : "Digester (AN) Efficiency (TODO)",
8 : "Urine Recyclability (TODO)",
9 : "Fraction Biomass to AE (TODO)",
10 : "Fraction Feces to AE (TODO)",
11 : "Fecal Fraction of Waster (TODO)",
12 : "Crew Members"}
...
def graph(variables):
    ...
    N_in = (.015)*(variables[2])*(variables[1])*(variables[3])*(n_x(variables[9], variables))**(Z)
    print(N_in)
    ...
    N_change = (variables[12] * x * (N_in + N_rec - N_out) ) / 14000

    print(N_change / 14000)
 
    plot = plt.figure()
    plt.plot(x, N_change, marker='o')
    plt.ylabel("ΔN (kg)")
    plt.xlabel("Time (d)")
    plt.title("Nitrogen Management Regiment Comparisons")
    plt.legend(["Combined NFR"])
    # plt.yticks(y)
    plt.show()
    plt.savefig("yo.png")
